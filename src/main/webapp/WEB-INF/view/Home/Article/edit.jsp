...
<rapid:override name="content">
    <blockquote class="layui-elem-quote">
        <span class="layui-breadcrumb" lay-separator="/">
              ...
              <a href="/back/article">文章列表</a>
              <a><cite>修改文章</cite></a>
        </span>
    </blockquote>


    <form class="layui-form" method="post" id="myForm" action="/home/article/editSubmit">
        ...
        <div class="layui-form-item">
            ...
            <div class="layui-input-inline">
                <select name="articleChildCategoryId" id="articleChildCategoryId" lay-filter="articleChildCategoryId">
                    <c:forEach items="${categoryList}" var="c">
                        <c:if test="${c.superCategoryId == article.categoryList[0].categoryId}">
                            <option value="${c.categoryId}"
                                    <c:if test="${article.categoryList[1].categoryId == c.categoryId}">selected</c:if>>${c.categoryName}
                            </option>
                        </c:if>
                    </c:forEach>
                </select>
    ...
    <blockquote class="layui-elem-quote layui-quote-nm">
        ...
        2、写文章之前，请确保分类存在；请勿刷新页面，博客不会自动保存 <br>
    </blockquote>
        ...
        layui.use(['form', 'layedit', 'laydate'], function () {
           ...
           layui.code({
            ...
            form.on("select(articleParentCategoryId)", function () {
                ...
                <c:forEach items="${categoryList}" var="c">
                if (articleParentCategoryId ==${c.superCategoryId}) {
